...
def list_scores(name):
    ...
    if name not in results:
        raw_input("There are no results for %s..." % name)
        return
    wins = results[name][0]
...
def play_game(name, board):
    while True:
        ...
        if not col or not row or not val:
            raw_input("Invalid format. You must enter a column, row, separator character and value in that order...")
            continue
        new_board = sudoku.update_board(board, col, row, val)
        if not new_board:
            raw_input("Unable to update board at %s%s with %s..." % (col.upper(), row, val))
            continue
        board = new_board
...
def show_menu():
    while True:
        ...
        if option == 'q':
            ...
            name = raw_input("Whose scores do you want to see? ")
            list_scores(name)
        else:
            raw_input("I didn't recognize that option...")
